    ...
    public function submitbonusamount($code)
    {
        ...
        if ($bonus) {
            if ($code == $bonus->code) {
                ...
                $checkBonusUses = BonusLedger::where('bonus_id',$bonus->id )->where('user_id', $user->id)->first();
                if ($checkBonusUses){
                    return redirect()->back()->with('success', 'Do not use again this code');
                }
                ...
                if ($bonus->counter < $bonus->set_service_counter) {
                    ...
                    return redirect()->back()->with('success', 'Try next.' );
                }
            } else {
                return redirect()->back()->with('success', 'Code is invalid.' );
            }
        } else {
            return redirect()->back()->with('success', 'Not Yet ready to use.');
        }
    }
}